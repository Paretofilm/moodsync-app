...
const schema = a.schema({
  ...
  UserProfile: a
    .model({
      ...
      moodVisibility: a.enum(['PUBLIC', 'FRIENDS_ONLY', 'PRIVATE']).default('FRIENDS_ONLY'),
  ...
  Mood: a
    .model({
      userId: a.string().required(),
      moodType: a.enum(['HAPPY', 'SAD', 'ENERGETIC', 'CALM', 'ANXIOUS']).required(),
      moodColor: a.enum(['YELLOW', 'BLUE', 'ORANGE', 'GREEN', 'PURPLE']).required(),
    ...
    .authorization((allow) => [
      ...
      allow.authenticated().to(['read']).where((mood) => mood.isPrivate.eq(false)), // Public moods
    ]),
  ...
  MoodActivity: a
    .model({
      userId: a.string().required(),
      activityType: a.enum(['MOOD_POSTED', 'COMMENT_ADDED', 'FRIEND_REQUEST_SENT', 'FRIEND_REQUEST_ACCEPTED']).required(),
